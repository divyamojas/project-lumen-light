...
Lumen is a local-first Progressive Web App journal built with Next.js 14 App Router, Tailwind CSS, and plain JavaScript. The current implementation is Phase 1 MVP only: entry creation, listing, detail view, deletion, JSON export, PWA manifest/service worker setup, and Dockerized development. There is no backend, no database, no authentication, and no analytics. All persistence is browser `localStorage` only.

## Full File Structure

- `app/globals.css` : Tailwind layers, base dark theme, and shared utility styles
- `app/layout.js` : Root layout, Google font loading, metadata, and global body shell
- `app/page.js` : Home screen with entry feed, export action, empty state, and floating new-entry trigger
- `app/entry/[id]/page.js` : Client-side detail route that reads one entry by ID, handles delete flow, and mirrors appearance controls
- `components/EntryCard.js` : Theme-aware list card with accent dot, preview text, and mount animation
- `components/EntryDetail.js` : Full entry presentation layout with back and delete actions
- `components/EntryEditor.js` : Controlled bottom-sheet modal for new entry creation; no `<form>` usage
- `components/ExportButton.js` : Browser-only JSON export using Blob and temporary anchor download
- `lib/storage.js` : Single source of truth for every `localStorage` read/write operation, including persisted UI mode
...
- `docker-compose.yml` : Compose service for live-reload local development on port 3000 with a named `node_modules` volume and explicit LAN host binding
...
- `CLAUDE.md` : This file; authoritative context snapshot for AI coding agents
...
- Light/dark appearance is separate from entry sentiment themeing; do not conflate the two systems
