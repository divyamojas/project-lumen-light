...
const postJson = async (url, body) => {
  ...
  const monitorId = beginApiRequest({
    ...
    requestBody: body ? JSON.stringify(body).slice(0, 2000) : null,
  });
...
const resolveUserFromToken = async (token) => {
  ...
  const monitorId = beginApiRequest({
    ...
    url,
    source: "auth",
    requestHeaders: { Authorization: `Bearer ${token.slice(0, 8)}…[redacted]` },
  });
...
export const resolveCurrentSession = async () => {
  ...
  try {
    ...
    try {
      const prefs = await requestJson(`${API_BASE}/users/me/preferences`);
      if (Array.isArray(prefs?.enabled_journal_types)) {
        setEnabledJournalTypes(prefs.enabled_journal_types);
      }
      if (prefs?.default_journal_type) {
        setDefaultJournalType(prefs.default_journal_type);
      }
    } catch (_e) {
      // Non-fatal
    }
