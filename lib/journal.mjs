...
export const PENDING_NOTICE_KEY = "lumen_pending_notice";
...
export const normalizeDraft = (draft) => {
  return {
    ...
    promptId: typeof draft?.promptId === "string" ? draft.promptId : "",
    relatedEntryIds: Array.isArray(draft?.relatedEntryIds)
      ? draft.relatedEntryIds.filter((value) => typeof value === "string").slice(0, 8)
      : [],
    journal_type: typeof draft?.journal_type === "string" && draft.journal_type ? draft.journal_type : "personal",
    type_metadata:
      draft?.type_metadata && typeof draft.type_metadata === "object" && !Array.isArray(draft.type_metadata)
        ? draft.type_metadata
        : {},
    theme: typeof draft?.theme === "string" && draft.theme ? draft.theme : "neutral",
  };
};

export const resolveEntryEditorRelatedIds = (draftRelatedEntryIds = [], relatedEntries = []) => {
  if (Array.isArray(draftRelatedEntryIds) && draftRelatedEntryIds.length > 0) {
    return draftRelatedEntryIds.filter((value) => typeof value === "string").slice(0, 8);
  }
...
export const savePendingNotice = (storage, message, key = PENDING_NOTICE_KEY) => {
  ...
  storage.setItem(key, nextMessage);
  return nextMessage;
};

export const consumePendingNotice = (storage, key = PENDING_NOTICE_KEY) => {
