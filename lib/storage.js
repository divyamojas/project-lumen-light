...
const HOST_SYNC_KEY = "lumen_host_sync_v1";
...
const ENTRY_API_PATH = "/api/entries";
const ENTRY_CACHE_MAX_AGE = 5000;
...
const shouldSyncLocalEntries = () => {
  if (!isBrowser()) {
    return false;
  }

  return window.localStorage.getItem(HOST_SYNC_KEY) !== "done";
};

const markLocalEntriesSynced = () => {
  if (!isBrowser()) {
    return;
  }

  window.localStorage.setItem(HOST_SYNC_KEY, "done");
};

const syncLocalEntriesToHost = async () => {
  if (!shouldSyncLocalEntries()) {
    return;
  }

  const localEntries = await getEntriesFromIndexedDb();
  ...
  await requestJson(`${ENTRY_API_PATH}/import`, {
    method: "POST",
    body: JSON.stringify({
...
export const getEntries = async () => {
  ...
  await syncLocalEntriesToHost();
  return updateEntriesCache(await requestJson(ENTRY_API_PATH));
};
...
export const saveEntries = async (entries) => {
  const result = await requestJson(`${ENTRY_API_PATH}/import`, {
    method: "POST",
    body: JSON.stringify({
      entries,
      duplicateMode: "replace-existing",
  ...
  return updateEntriesCache(result.entries);
};
...
export const createEntry = async ({
  ...
  const cachedEntries = getCachedEntries(ENTRY_CACHE_MAX_AGE * 3) || [];
  updateEntriesCache([entry, ...cachedEntries.filter((cachedEntry) => cachedEntry.id !== entry.id)]);
  return entry;
};
...
export const updateEntry = async (id, updates) => {
  ...
  if (cachedEntries) {
    updateEntriesCache(cachedEntries.map((cachedEntry) => (cachedEntry.id === id ? entry : cachedEntry)));
  }
...
export const deleteEntry = async (id) => {
  const result = await requestJson(`${ENTRY_API_PATH}/${id}`, {
    method: "DELETE",
  });
...
export const importEntries = async (importedEntries, options = {}) => {
  ...
  updateEntriesCache(result.entries);
  return result;
};
