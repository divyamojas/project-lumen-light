...
const SESSION_COOKIE_KEY = "lumen_session";
const LEGACY_AUTH_COOKIE_KEY = "lumen_auth";
...
export const saveAuthCookie = (token) => {
  ...
  document.cookie = `${SESSION_COOKIE_KEY}=1; path=/; max-age=${AUTH_COOKIE_MAX_AGE}; SameSite=Lax; Secure`;
  clearCookie(LEGACY_AUTH_COOKIE_KEY);
  return nextToken;
};
...
export const requestJson = async (path, options = {}) => {
  ...
  const monitorHeaders = token
    ? { ...fetchHeaders, Authorization: `Bearer ${token.slice(0, 8)}…[redacted]` }
    : fetchHeaders;
  const monitorId = beginApiRequest({
    method,
    url: path,
    source: "requestJson",
    requestHeaders: monitorHeaders,
    requestBody: options.body ? String(options.body).slice(0, 2000) : null,
  });
  ...
  finishApiRequest(monitorId, {
    statusCode: response.status,
    responseSnippet: JSON.stringify(payload).slice(0, 2000),
  });
  return payload;
};
